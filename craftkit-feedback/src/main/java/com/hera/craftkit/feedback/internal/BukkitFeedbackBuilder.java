...
public final class BukkitFeedbackBuilder {
    ...
    public BukkitFeedbackBuilder defaultLanguage(String defaultLanguage) {
        String normalized = FeedbackTranslations.normalizeLanguage(defaultLanguage);
        if (normalized.isBlank()) {
            throw new IllegalArgumentException("defaultLanguage cannot be blank");
        }
    ...
    public Feedback build() {
        if (!networkPlayerSettingsRequired) {
            throw new IllegalStateException("NetworkPlayerSettings is required by craftkit-feedback. Call networkPlayerSettingsRequired().");
        }
        if (translations == null) {
            throw new IllegalStateException("translations are required");
        }
        if (!translations.supports(defaultLanguage)) {
            throw new IllegalStateException("Default language '" + defaultLanguage + "' has no translation document.");
        }

        PlayerSettingsService settings = plugin.getServer().getServicesManager().load(PlayerSettingsService.class);
        if (settings == null) {
            throw new FeedbackMissingDependencyException("NetworkPlayerSettings PlayerSettingsService is not registered. Ensure the consumer plugin depends on NetworkPlayerSettings and builds Feedback after it is enabled.");
        }
        ...
        return new DefaultFeedback(plugin, defaultLanguage, translations, resolver);
    }
}
